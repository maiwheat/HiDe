...
def tune_hyperparams_on_val_tasks(args, cls_order, config_generic={}, config_model={}, config_cl={}):
    ...
    resources = {'cpu': 4, 'gpu': 0.25} if args.device == 'cuda' else {'cpu': 1}
    # resources = {'cpu': 2, 'gpu': 0.25} if args.device == 'cuda' else {'cpu': 1}
    ...
    for run in range(args.runs):
        ...
        if args.agent == 'Offline':
            ...
            for i in range(n_tasks_exp):
                ...
                # # 保存第一个任务学完后的教师模型
                # if i == 0:
                #     teacher_model_path = args.exp_path + '/teacher_after_task0_r{}.pt'.format(run)
                #     torch.save(agent.model.state_dict(), teacher_model_path)
                #     print('\n >>> 已保存第一个任务后的教师模型到: {}'.format(teacher_model_path))
                ...
                if i+1 == n_tasks_exp and args.cf_matrix:
                    ...
                    # 使用实际学习的类别数，而不是 task_stream.n_classes
                    actual_n_classes = len(agent.learned_classes)
                    agent.plot_cf_matrix(path=cf_matrix_path, classes=np.arange(actual_n_classes))

            # # 保存最终学好的模型
            # final_model_path = args.exp_path + '/final_model_r{}.pt'.format(run)
            # torch.save(agent.model.state_dict(), final_model_path)
            # print('\n >>> 已保存最终模型到: {}'.format(final_model_path))

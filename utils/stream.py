...
class IncrementalTaskStream(object):
    """
        将总的任务划分到一个数组当中，数组中含有每个任务的训练集、验证集和测试集
    """
    def __init__(self, data, scenario, cls_order, split):
        ...
        if self.split == 'all':  # Decide which task stream to use, Exp or Val or All
            ...
            # 对于小数据集(har, uwave, sines)，使用 n_tasks_val 作为任务数
            # end = n_tasks_val[data] if n_tasks[self.data] < 5 else n_tasks[data]
        else:
    ...
    def load_data(self, load_subject=False):
        ...
        if (self.data != 'emg' and n_tasks[self.data] < 5 or self.data == 'charactertrajectories')   and self.split == 'val':
            x_test = load_pickle(self.path + 'x_val.pkl')
            y_test = load_pickle(self.path + 'state_val.pkl')
        # squeeze：去掉shape中的1的维度
    ...
    def setup(self, cut=0.9, load_subject=False):
        ...
        for t in range(self.n_tasks):
            ...
            # 将任务对应的类标签添加到classes_in_task_t中
            ...
            if load_subject:
                ...
                # classes_in_task_t 为任务对应的类标签
                # x_train, y_train为训练集数据和标签
                # extract_samples_according_to_labels：根据类标签提取数据
                # x_train_t, y_train_t为任务t对应的训练集数据和标签
                x_train_t, y_train_t = extract_samples_according_to_labels(x_train, y_train, classes_in_task_t)
            # x_test_t, y_test_t为任务t对应的测试集数据和标签

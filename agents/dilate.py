...
class DILATE(BaseLearner):

    def __init__(self, model, args):
        super(DILATE, self).__init__(model, args)
        self.use_kd = True
    ...
    def train_epoch(self, dataloader, epoch):
        ...
        elif self.fmap_kd_metric == 'dilate':
            similarity_metric = dilate_loss
        else:
        ...
        for batch_id, (x, y) in enumerate(dataloader):
            ...
            if self.task_now == 0:
                ...
                if self.fmap_kd_metric == 'dtw' or self.fmap_kd_metric == 'dilate':
                    student_fmap = student_fmap.permute(0, 2, 1)  # S-DTW requires shape of (N, L, D)
                    teacher_fmap = teacher_fmap.permute(0, 2, 1)
                if self.fmap_kd_metric == 'dilate':
                    loss_kd_fmap = similarity_metric(student_fmap, teacher_fmap, self.alpha, 1, self.device)
                else:
    ...
    def after_task(self, x_train, y_train):
        super(DILATE, self).after_task(x_train, y_train)

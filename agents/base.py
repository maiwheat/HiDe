# 类增量学习方法的基类，用于定义一些通用的方法，如训练一个任务、评估模型、绘制混淆矩阵等。
# 子类继承 BaseLearner 类的方法，需要实现 train_epoch 方法，用于训练一个epoch。
...
class BaseLearner(nn.Module, metaclass=abc.ABCMeta):
    ...
    def before_task(self, y_train):
        ...
        if self.task_now != 0:
            ...
            # optimizer 只会跟踪 最初模型中的参数，如果模型新增了参数（比如新增 n_new_classes 个输出神经元），旧的 optimizer 并不会自动优化这些新参数，导致：
            #
            # 新类别无法学习：因为 optimizer 没有更新权重 W_new 的梯度。
            #
            # 模型可能崩溃：optimizer 可能只优化 old_classes 的权重，但 new_classes 的梯度未被计算，训练可能出现 NaN 或梯度爆炸。
    ...
    def learn_task(self, task):
        ...
        # optimizer 负责更新模型参数（例如 SGD、Adam）。
        # scheduler 负责调整学习率 (lr)，优化 optimizer 的学习效果。
        ...
        for epoch in range(self.epochs):
            # Train for one epoch 调用子类的 train_epoch 方法，计算训练集每轮的损失和准确率
            epoch_loss_train, epoch_acc_train = self.train_epoch(train_dataloader, epoch=epoch)

            # Test on val set for early stop 计算验证集上的损失和准确率
            ...
            # 通过在验证集上的损失是否在连续训练论数保持不变来判断是否需要提前停止，每次传入验证集损失，由EarlyStopping类中的counter判断是否需要提前停止
    ...
    def evaluate(self, task_stream, path=None):
        ...
        Args:
            ...
            TSNE 在这里指的是 t-分布随机邻居嵌入 (t-Distributed Stochastic Neighbor Embedding, t-SNE)，它是一种降维与可视化技术，用于将高维特征映射到 2D 或 3D 平面，从而观察模型学到的特征分布情况。
        """
        ...
        for mode in eval_modes:
            ...
            for i in range(self.task_now + 1):
                ...
                # 如果是最后一个任务，且需要绘制混淆矩阵和测试集的情况下则绘制混淆矩阵，否则输出测试集的准确率和损失
